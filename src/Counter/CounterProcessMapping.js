...
export const CounterForm = () => {
    ...
    useEffect(() => {
        ...
        setProcessStages(param?.state?.processStages);
        param?.state?.data?.mappedProcessStages?.map((stage) => (setSelectedStages(current => [...current, {value: stage.code, label: stage.name}])));
    ...
    const handleSubmit = (event) => {
        ...
        event.preventDefault();
            if(orgCode === '') {
                ...
                let selectedProcessStages = [];
                selectedStages?.map((stage) => (selectedProcessStages.push(stage.value)));
                console.log("Process Stages: ",selectedProcessStages);
                dispatch(counterActions.addProcessMap(counterNo,{
                    ...
                    counterCode: code,
                    processStage: selectedProcessStages
                }));
            }
    ...
    const handleChange = (event) => {
        setSelectedStages([]);
        event.map((stage) => (setSelectedStages(current => [...current, stage])));
    };
                            ...
                            <div className="row form-row">
                                ...
                                <div className="col-md-6">
                                    <div className="form-group">
                                        <div className="input-group mb-3">
                                            <div className="input-group-prepend">
                                                <span className="input-group-text required" id="processStage">Process Stage</span>
                                            </div>
                                            <Select
                                                className="custom-select form-control form-control-1 header-drop-down float-right"
                                                style={{padding: '0px !important'}}
